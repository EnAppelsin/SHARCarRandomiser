...
if Midx ~= nil then
	if GetSetting("RandomCharacter") then
		NewFile = string.gsub(NewFile, "SetDialogueInfo%s*%(%s*\"(.-)\"%s*,%s*\"(.-)\"", function(char1, char2)
			if char1 == OrigChar then
				char1 = RandomChar
			elseif char2 == OrigChar then
				char2 = RandomChar
			end
			return "SetDialogueInfo(\"" .. char1 .. "\",\"" .. char2 .. "\""
		end)
