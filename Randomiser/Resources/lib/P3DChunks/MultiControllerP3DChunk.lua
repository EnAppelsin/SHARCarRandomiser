...
function P3D.MultiControllerP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Name, chunk.Version, chunk.Length, chunk.Framerate = string_unpack(Endian .. "s1Iff", chunk.ValueStr)
...
function P3D.MultiControllerP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1IffI", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, self.Length, self.Framerate, self:GetNumTracks()) .. chunkData
end