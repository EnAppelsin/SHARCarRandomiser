...
function P3D.CollisionOrientedBoundingBoxP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.HalfExtents = {}
	chunk.HalfExtents.X, chunk.HalfExtents.Y, chunk.HalfExtents.Z = string_unpack(Endian .. "fff", chunk.ValueStr)
...
function P3D.CollisionOrientedBoundingBoxP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIfff", self.Identifier, headerLen, headerLen + #chunkData, self.HalfExtents.X, self.HalfExtents.Y, self.HalfExtents.Z) .. chunkData
end