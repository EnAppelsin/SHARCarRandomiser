...
function P3D.TreeNodeP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.NumChildren, chunk.ParentOffset = string_unpack(Endian .. "Ii", chunk.ValueStr)
...
function P3D.TreeNodeP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIIi", self.Identifier, headerLen, headerLen + #chunkData, self.NumChildren, self.ParentOffset) .. chunkData
end