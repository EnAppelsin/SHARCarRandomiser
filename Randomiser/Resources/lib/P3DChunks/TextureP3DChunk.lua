...
function P3D.TextureP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Name, chunk.Version, chunk.Width, chunk.Height, chunk.Bpp, chunk.AlphaDepth, chunk.NumMipMaps, chunk.TextureType, chunk.Usage, chunk.Priority = string_unpack(Endian .. "s1IIIIIIIII", chunk.ValueStr)
...
function P3D.TextureP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1IIIIIIIII", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, self.Width, self.Height, self.Bpp, self.AlphaDepth, self.NumMipMaps, self.TextureType, self.Usage, self.Priority) .. chunkData
end