...
function P3D.ImageDataP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.ImageData = string_unpack(Endian .. "s4", chunk.ValueStr)
...
function P3D.ImageDataP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs4", self.Identifier, headerLen, headerLen + #chunkData, self.ImageData) .. chunkData
end