...
function P3D.FrontendTextStyleResourceP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Name, chunk.Version, chunk.Filename, chunk.InventoryName = string_unpack(Endian .. "s1Is1s1", chunk.ValueStr)
...
function P3D.FrontendTextStyleResourceP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1Is1s1", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, Filename, InventoryName) .. chunkData
end