...
function P3D.VolumeImageP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Name, chunk.Version, chunk.Width, chunk.Height, chunk.Depth, chunk.Bpp, chunk.Palettized, chunk.HasAlpha, chunk.Format = string_unpack(Endian .. "s1IIIIIIII", chunk.ValueStr)
...
function P3D.VolumeImageP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1IIIIIIII", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, self.Width, self.Height, self.Depth, self.Bpp, self.Palettized, self.HasAlpha, self.Format) .. chunkData
end