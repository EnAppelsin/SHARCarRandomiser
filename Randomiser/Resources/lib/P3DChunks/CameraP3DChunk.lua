...
function P3D.CameraP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Position = {}
	chunk.Look = {}
	chunk.Up = {}
	chunk.Name, chunk.Version, chunk.FOV, chunk.AspectRatio, chunk.NearClip, chunk.FarClip, chunk.Position.X, chunk.Position.Y, chunk.Position.Z, chunk.Look.X, chunk.Look.Y, chunk.Look.Z, chunk.Up.X, chunk.Up.Y, chunk.Up.Z = string_unpack(Endian .. "s1Ifffffffffffff", chunk.ValueStr)
...
function P3D.CameraP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1Ifffffffffffff", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, self.FOV, self.AspectRatio, self.NearClip, self.FarClip, self.Position.X, self.Position.Y, self.Position.Z, self.Look.X, self.Look.Y, self.Look.Z, self.Up.X, self.Up.Y, self.Up.Z) .. chunkData
end