...
function P3D.FollowCameraDataP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.TargetOffset = {}
	chunk.Index, chunk.Rotation, chunk.Elevation, chunk.Magnitude, chunk.TargetOffset.X, chunk.TargetOffset.Y, chunk.TargetOffset.Z = string_unpack(Endian .. "Iffffff", chunk.ValueStr)
...
function P3D.FollowCameraDataP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIIffffff", self.Identifier, headerLen, headerLen + #chunkData, self.Index, self.Rotation, self.Elevation, self.Magnitude, self.TargetOffset.X, self.TargetOffset.Y, self.TargetOffset.Z) .. chunkData
end