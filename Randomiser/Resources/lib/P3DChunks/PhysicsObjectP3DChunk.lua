...
function P3D.PhysicsObjectP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	local pos
	chunk.Name, chunk.Version, chunk.MaterialName, chunk.NumJoints, chunk.Volume, chunk.RestingSensitivity, pos = string_unpack(Endian .. "s1Is1Iff", chunk.ValueStr)
...
function P3D.PhysicsObjectP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1Is1Iff", self.Identifier, headerLen, headerLen + #chunkData, Name, self.Version, MaterialName, self.NumJoints, self.Volume, self.RestingSensitivity) .. chunkData
end