...
function P3D.StatePropEventDataP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.Name, chunk.State, chunk.EventEnum = string_unpack(Endian .. "s1Ii", chunk.ValueStr)
...
function P3D.StatePropEventDataP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1Ii", self.Identifier, headerLen, headerLen + #chunkData, Name, self.State, self.EventEnum) .. chunkData
end