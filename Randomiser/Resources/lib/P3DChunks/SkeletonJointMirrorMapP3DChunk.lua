...
function P3D.SkeletonJointMirrorMapP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.MappedJointIndex, chunk.XAxisMap, chunk.YAxisMap, chunk.ZAxisMap = string_unpack(Endian .. "Ifff", chunk.ValueStr)
...
function P3D.SkeletonJointMirrorMapP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIIfff", self.Identifier, headerLen, headerLen + #chunkData, self.MappedJointIndex, self.XAxisMap, self.YAxisMap, self.ZAxisMap) .. chunkData
end