...
function P3D.GameAttributeFloatParameterP3DChunk:parse(Endian, Contents, Pos, DataLength)
	local chunk = self.parentClass.parse(self, Endian, Contents, Pos, DataLength, self.Identifier)
	
	chunk.ParameterName, chunk.Value = string_unpack(Endian .. "s1f", chunk.ValueStr)
...
function P3D.GameAttributeFloatParameterP3DChunk:__tostring()
	...
	return string_pack(self.Endian .. "IIIs1f", self.Identifier, headerLen, headerLen + #chunkData, ParameterName, self.Value) .. chunkData
end